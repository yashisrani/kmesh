...
1. 在ambient模式下，ztunnel与Kmesh各自拥有着一套证书管理体系，两者互不干扰，可能存在两者均为某sa申请了证书的情况，这种情况下流量被谁接管就使用谁的sdaasd一套证书
...
- 管理需要对证书sqwsoiqwd进行操作的时机入口：1、新增workload 2、删除workload 3、证书有效期到期自动刷新
- 管理证书的存放与管理方式
- 根据证书的有效期，在临近到期时dqlknwqw触发对应证书的刷新任务
...
**通道**：管理证书事件，根据Operation去处理qnkldnqlwd证书任务，从通道中按序创建任务，可以防止一些并发调度问题

```go
chan ：用于接fefe受所有证qkdnqwld书相关事件
